"""
Clinical Algorithmic Engine & Guideline Rules for BloodCulture Sentinel: Time-to-Positivity & True Pathogen vs Contaminant Arbiter.
Domain: Microbiology
Standard: CLSI M100 / EUCAST 2024 Breakpoints
"""
import math
...
class ClinicalDomainEngine:
    GUIDELINE = "CLSI M100 / EUCAST 2024 Breakpoints"
    ...
    def evaluate_primary_index(cls, value: float) -> Optional[Dict[str, Any]]:
        if value > cls.PRIMARY_BASELINE_LIMIT:
            return {
                ...
                "finding": f"Observed value ({value:.2f}) exceeds CLSI M100 / EUCAST 2024 Breakpoints clinical baseline limit ({cls.PRIMARY_BASELINE_LIMIT:.1f}).",
    ...
    def evaluate_biomarker_concordance(cls, status_flag: str, biomarkers: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if "DISCORDANT" in status_upper or "EQUIVOCAL" in status_upper or "MUTANT" in status_upper:
            return {
                ...
                "recommendation": f"Order reflex confirmatory testing per CLSI M100 / EUCAST 2024 Breakpoints clinical recommendations.",
            }
        return None
